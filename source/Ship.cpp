...
bool Ship::Move(list<Effect> &effects, list<shared_ptr<Flotsam>> &flotsam)
{
	...
	isDisabled = isOverheated || hull < MinimumHull() || (!crew && requiredCrew) || heat < -300 * Mass();
...
shared_ptr<Ship> Ship::Board(bool autoPlunder)
{
	...
	if(!government->IsEnemy(victim->GetGovernment()))
	{
		...
		if(!victim->GetPersonality().IsDerelict())
            victim->shields = max(victim->shields, victim->MinimumShields());
...
void Ship::WasCaptured(const shared_ptr<Ship> &capturer)
{
	...
	shields = max(shields, MinimumShields());
...
double Ship::MinimumShields() const
{
    if(neverDisabled)
        return 0.;

    double maximumShields = attributes.Get("shields");
    return max(.05 * maximumShields, min(.25 * maximumShields, 300.));
}
